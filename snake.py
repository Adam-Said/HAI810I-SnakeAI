...
class SnakeGame:
    ...
    def _update_ui(self):
        ...
        for pt in self.snake:
            ...
            pygame.draw.rect(self.display, SNAKE_COLOR_BODY,
                             pygame.Rect(pt.x + (SHADOW_SIZE / 2), pt.y + (SHADOW_SIZE / 2), BLOCK_SIZE - SHADOW_SIZE,
                                         BLOCK_SIZE - SHADOW_SIZE))
            ...
            if is_head:
                ...
                if self.direction == Direction.RIGHT:
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (2 * BLOCK_SIZE / 3), pt.y + (1 * EYES_SIZE), EYES_SIZE,
                                                 EYES_SIZE))
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (2 * BLOCK_SIZE / 3), pt.y + (3 * EYES_SIZE), EYES_SIZE,
                                                 EYES_SIZE))
                    pygame.draw.rect(self.display, FRUIT_COLOR,
                                     pygame.Rect(pt.x + BLOCK_SIZE, pt.y + (BLOCK_SIZE / 2) - 1, TONGUE_SIZE_LONG,
                                                 TONGUE_SIZE_LARG))
                elif self.direction == Direction.LEFT:
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (1 * BLOCK_SIZE / 3) - EYES_SIZE, pt.y + (1 * EYES_SIZE),
                                                 EYES_SIZE, EYES_SIZE))
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (1 * BLOCK_SIZE / 3) - EYES_SIZE, pt.y + (3 * EYES_SIZE),
                                                 EYES_SIZE, EYES_SIZE))
                    pygame.draw.rect(self.display, FRUIT_COLOR,
                                     pygame.Rect(pt.x - TONGUE_SIZE_LONG, pt.y + (BLOCK_SIZE / 2) - 1, TONGUE_SIZE_LONG,
                                                 TONGUE_SIZE_LARG))
                elif self.direction == Direction.DOWN:
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (1 * EYES_SIZE), pt.y + (2 * BLOCK_SIZE / 3), EYES_SIZE,
                                                 EYES_SIZE))
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (3 * EYES_SIZE), pt.y + (2 * BLOCK_SIZE / 3), EYES_SIZE,
                                                 EYES_SIZE))
                    pygame.draw.rect(self.display, FRUIT_COLOR,
                                     pygame.Rect(pt.x + (BLOCK_SIZE / 2) - 1, pt.y + BLOCK_SIZE, TONGUE_SIZE_LARG,
                                                 TONGUE_SIZE_LONG))
                elif self.direction == Direction.UP:
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (1 * EYES_SIZE), pt.y + (1 * BLOCK_SIZE / 3) - EYES_SIZE,
                                                 EYES_SIZE, EYES_SIZE))
                    pygame.draw.rect(self.display, SNAKE_COLOR_EYES,
                                     pygame.Rect(pt.x + (3 * EYES_SIZE), pt.y + (1 * BLOCK_SIZE / 3) - EYES_SIZE,
                                                 EYES_SIZE, EYES_SIZE))
                    pygame.draw.rect(self.display, FRUIT_COLOR,
                                     pygame.Rect(pt.x + (BLOCK_SIZE / 2) - 1, pt.y - TONGUE_SIZE_LONG, TONGUE_SIZE_LARG,
                                                 TONGUE_SIZE_LONG))

        pygame.draw.circle(self.display, FRUIT_COLOR, [self.food.x + DEMI_BLOCK_SIZE, self.food.y + DEMI_BLOCK_SIZE],
                           DEMI_BLOCK_SIZE)
